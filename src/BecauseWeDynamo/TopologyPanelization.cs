...
namespace Topology.Test.Panelization
{
    public class Panel<T>: IDisposable where T: Topology.Face, new()
    {
        ...
        //**CREATE
        public static Panel<T> ByFaceAndParameters(T Face, double ThicknessFront, double ThicknessBack, double MinEdgeOffset, double CornerRadius, double BevelAngle = 0)
        { return new Panel<T>(Face, ThicknessFront, ThicknessBack, MinEdgeOffset, CornerRadius, BevelAngle); }
        ...
        public PolyCurve[] GetLabels(double Scale, PanelSystem<T> PanelSystem)
        {
            List<PolyCurve> labels = new List<PolyCurve>();
            for (int j = 0; j < Face.E.Count; j++)
            {
                ...
                Point m = Point.ByCoordinates(ArcPoints[j][2].X / 2 + ArcPoints[(j + 1) % 3][0].X / 2, ArcPoints[j][2].Y / 2 + ArcPoints[(j + 1) % 3][0].Y / 2, ArcPoints[j][2].Z / 2 + ArcPoints[(j + 1) % 3][0].Z / 2);
                Vector Y = Face.VertexVectors[j][0].Cross(Face.Normal);
                m = m.Subtract(Y.Scale(PanelSystem.E[Face.Edges[j]].Inset + 2 * PanelSystem.E[Face.Edges[j]].Dim['S']));
                Word w = Word.ByStringOriginVectors(Face.E[j].Edge.Name, m, Face.VertexVectors[j][0], Y);
                labels.AddRange(w.display(Scale));
                m.Dispose(); Y.Dispose(); w.Dispose();
            }
    ...
    public class EdgeConnector : IDisposable
    {
        ...
        internal EdgeConnector(HalfEdge e1, HalfEdge e2, double Height, double Depth, double Spacing, double ThicknessFront, double ThicknessBack, double PanelMinOffset)
        {
            ...
            if (i1 + i2 != 2)
            {
                ...
                // v1: farthest point in first halfedge face direction (inset + 1.5 spacing)
                P1.Add(X1.Scale(-ThicknessBack / 2).Add(Y1.Normalized().Scale(Inset + Dim['S'] + Dim['H']/2)));
                // v2: point to create square edge with face and beginning of fillet arc
                P1.Add(P1[1].Add(X1.Scale(-Dim['H'] / 2)));
                // v3: center of fillet arc
                P1.Add(P1[2].Add(Y1.Normalized().Scale(-Dim['H'] / 2)));
                // v4: end of fillet arc;
                ...
                // w0: farthest point in second halfedge face direction (inset + 1.5 spacing)
                P2.Add(X2.Scale(-ThicknessBack / 2).Add(Y2.Scale(Inset + Dim['S'] + Dim['H'] / 2)));
                // w1: point to create square edge with face and beginning of fillet arc
                P2.Add(P2[0].Add(X2.Scale(-Dim['H'] / 2)));
                // w2: center of fillet arc
                P2.Add(P2[1].Add(Y2.Normalized().Scale(-Dim['H'] / 2)));
                // w3: end of fillet arc
    ...
    public class PanelSystem<T> : IDisposable where T:Topology.Face, new()
    {
        //**FIELDS
        bool disposed = false;
        internal Dictionary<T, Panel<T>> F;
        internal Dictionary<Topology.Edge, EdgeConnector> E;
        internal Mesh<T> M;

        //**PROPERTIES**QUERY
        public Panel<T>[] Panels { get { return F.Values.ToArray(); } }
        public EdgeConnector[] Connectors { get { return E.Values.ToArray(); } }

        //**CONSTRUCTOR
        internal PanelSystem(Mesh<T> Mesh) { M = Mesh; }
        internal PanelSystem(Mesh<T> Mesh, double Height, double Depth, double Spacing, double ThicknessFront, double ThicknessBack, double MinEdgeOffset, double CornerRadius, double BevelAngle)
            : this(Mesh)
        ...
        //**METHODS**CREATE
        public static PanelSystem<T> ByMesh(Mesh<T> Mesh) { return new PanelSystem<T>(Mesh); }
        public static PanelSystem<T> ByMeshParameters(Mesh<T> Mesh, double Height, double Depth, double Spacing, double ThicknessFront, double ThicknessBack, double MinEdgeOffset, double CornerRadius, double BevelAngle)
        { return new PanelSystem<T>(Mesh, Height, Depth, Spacing, ThicknessFront, ThicknessBack, MinEdgeOffset, CornerRadius, BevelAngle); }
        ...
        public bool GeneratePanels(double ThicknessFront, double ThicknessBack, double MinEdgeOffset, double CornerRadius, double BevelAngle)
        {
            ...
            // initialize mesh face to panel dictionary
            F = new Dictionary<T, Panel<T>>(M.Faces.Count);
            // iterate through mesh faces to generate panels
            for (int i = 0; i < M.Faces.Count; i++)
            {
                // generate triangle panel with input values at given face
                Panel<T> t = Panel<T>.ByFaceAndParameters(M.Faces[i], ThicknessFront, ThicknessBack, MinEdgeOffset, CornerRadius, BevelAngle);
                // add face/panel pair to dictionary
                F.Add(M.Faces[i], t);
                // iterate through edges and add holes based on edge connector calculations
                for (int j = 0; j < M.Faces[i].Edges.Length; j++)
