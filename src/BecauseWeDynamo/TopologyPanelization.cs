...
namespace Test.Panelization
{
    public class Panel<T>: IDisposable where T: Topology.Face
    {
        ...
        //**CONSTRUCTOR
        internal Panel(T Face, double ThicknessFront, double ThicknessBack, double MinEdgeOffset, double CornerRadius, double BevelAngle)
        {
            // initialize
            this.Face = Face;
            ...
            for (int i = 0; i < Face.E.Count; i++)
            {
                ...
                if (Face.E[i].Angle < 180)
                {
                    if (Face.E[i].Angle >= 2*(90 - BevelAngle)) continue;
        ...
        {
            if (ArcPoints.Equals(null)) return null;
            Curve[] Curves = new Curve[ArcPoints.Length];
            for (int i =0; i< ArcPoints.Length; i++)
            {
                Curves[2*i] = Arc.ByThreePoints(ArcPoints[i][0],ArcPoints[i][1], ArcPoints[i][2]);
                Curves[2 * i + 1] = Line.ByStartPointEndPoint(ArcPoints[i][2], ArcPoints[(i + 1) % ArcPoints.Length][0]);
            }
            PolyCurve Profile = PolyCurve.ByJoinedCurves(Curves);
            Curves.ForEach(c => c.Dispose());
            return Profile;
        ...
        public Solid GetPanelSolid()
        {
            ...
            Vector Inset = Face.Normal.Cross(Face.E[0].GetVector().Normalized()).Normalized();
            Point[] pts = new Point[]{c.StartPoint.Add(Face.Normal.Scale(Thickness[0])),
                              c.StartPoint,
                              c.StartPoint.Add(Face.Normal.Scale(Thickness[1])).Add(Inset.Scale(BevelInset))};
            Curve l = PolyCurve.ByPoints(pts);
            Solid s = c.SweepAsSolid(l);
            c.Dispose(); Inset.Dispose(); l.Dispose(); pts.ForEach(p => p.Dispose());
            return s;
        }
        /// <summary>
        /// returns edge labels at edge on triangular mesh face backside as an Polycurve Array
        ...
        /// <param name="PanelSystem">parent PanelSystem of Panel</param>
        /// <returns>Polycurve Array</returns>
        public PolyCurve[] GetLabels(double Scale, TrianglePanelSystem PanelSystem)
        {
            List<PolyCurve> labels = new List<PolyCurve>();
            for (int j = 0; j < Face.E.Count; j++)
            {
    ...
    public class PanelHole<T> : Panel<T> where T : Topology.Face
    {
        public List<Circle> Holes { get; set; }
        
        //**METHODS**CONSTRUCTOR
        internal PanelHole(T Face, double ThicknessFront, double ThicknessBack, double MinEdgeOffset, double CornerRadius)
        ...
        public Solid GetPanelSolidHole()
        {
            if (Holes.Count < 1) return GetPanelSolid();
            Surface s = GetPanelSurface();
            Solid S = null;
            if (Holes.Count > 5)
            {
    ...
    public class PanelBevel: Panel<Topology.Face>
    {
        //**METHODS**CONSTRUCTOR
        internal PanelBevel(Topology.Face Face, double ThicknessFront, double ThicknessBack, double MinEdgeOffset, double CornerRadius, double BevelAngle = 0)
            : base(Face, ThicknessFront, ThicknessBack, MinEdgeOffset, CornerRadius, BevelAngle) {}

        //**METHOD**CREATE
        public static PanelBevel ByMeshFaceAndParameters(Triangle Triangle, double ThicknessFront, double ThicknessBack, double MinEdgeOffset, double CornerRadius, double BevelAngle)
        { return new PanelBevel(Triangle, ThicknessFront, ThicknessBack, MinEdgeOffset, CornerRadius, BevelAngle); }
    }

    public class TrianglePanel : PanelHole<Triangle>
    {
        internal TrianglePanel(Triangle Triangle, double ThicknessFront, double ThicknessBack, double MinEdgeOffset, double CornerRadius)
            : base(Triangle, ThicknessFront, ThicknessBack, MinEdgeOffset, CornerRadius) { }
        public static TrianglePanel ByTriangleAndParameters(Triangle Triangle, double ThicknessFront, double ThicknessBack, double MinEdgeOffset, double CornerRadius)
        { return new TrianglePanel(Triangle, ThicknessFront, ThicknessBack, MinEdgeOffset, CornerRadius); }
    }
    ...
    /// PanelSystem: Mesh based system of EdgeConnectors and TrianglePanels
    /// </summary>
    public class TrianglePanelSystem : IDisposable
    {
        //**FIELDS
        bool disposed = false;
        internal Dictionary<Triangle, TrianglePanel> T;
        ...
        //**CONSTRUCTOR
        internal TrianglePanelSystem(TriangleMesh Mesh) { M = Mesh; }
        internal TrianglePanelSystem(TriangleMesh Mesh, double Width, double Thickness, double MinEdgeOffset, double CornerRadius, double HoleRadius, double PocketRadius)
            : this(Mesh)
        { GenerateEdgeConnectors(Width, Thickness, MinEdgeOffset, PocketRadius); GenerateTrianglePanels(Thickness, MinEdgeOffset, CornerRadius, HoleRadius); }
        ...
        /// <param name="PocketRadius">Radius of pockets for EdgeConnectors</param>
        /// <returns>PanelSystem</returns>
        public static TrianglePanelSystem ByDefault(TriangleMesh Mesh) { return new TrianglePanelSystem(Mesh); }
        public static TrianglePanelSystem ByMesh(TriangleMesh Mesh, double Width, double Thickness, double MinEdgeOffset, double CornerRadius, double HoleRadius, double PocketRadius)
        { return new TrianglePanelSystem(Mesh, Width, Thickness, MinEdgeOffset, CornerRadius, HoleRadius, PocketRadius); }
        ...
        public TrianglePanel[] GenerateTrianglePanels(double Thickness, double MinEdgeOffset, double CornerRadius, double HoleRadius)
        {
            ...
            {
                // generate triangle panel with input values at given face
                TrianglePanel t = TrianglePanel.ByTriangleAndParameters(M.Faces[i], Thickness/2, Thickness/2, MinEdgeOffset, CornerRadius);
                // add face/panel pair to dictionary
                T.Add(M.Faces[i], t);
                // iterate through edges and add holes based on edge connector calculations
                for (int j = 0; j < M.Faces[i].Edges.Length; j++)
    ...
    /*public class PolygonPanelSystem: IDisposable
    {
         //**FIELDS
        bool disposed = false;
        internal Dictionary<Topology.Face, PanelBevel> F;
        internal Dictionary<Topology.Edge, EdgeConnector[]> E;
        internal TriangleMesh M;
        ...
        /// gets TrianglePanel Array based on mesh
        /// </summary>
        public PanelBevel[] Panels { get { return F.Values.ToArray(); } }
        /// <summary>
        /// gets two dimensional EdgeConnector Array indexed by mesh edge then by halfedge normals;
        /// each edge can have more than one EdgeConnector
        /// </summary>
        ...
        //**CONSTRUCTOR
        internal PolygonPanelSystem(PolyMesh Mesh) {}
        internal PolygonPanelSystem(PolygonPanelSystem Mesh, double Width, double Thickness, double MinEdgeOffset, double CornerRadius, double HoleRadius, double PocketRadius)
            : this(Mesh)
        { GenerateEdgeConnectors(Width, Thickness, MinEdgeOffset, PocketRadius); GenerateTrianglePanels(Thickness, MinEdgeOffset, CornerRadius, HoleRadius); }
